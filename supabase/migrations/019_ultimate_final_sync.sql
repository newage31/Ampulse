-- Migration finale ULTIME pour synchroniser les données avec seed.sql
-- Utilisation des vrais IDs existants

-- Supprimer toutes les données dépendantes pour éviter les conflits
DELETE FROM processus_reservations;
DELETE FROM reservations;
DELETE FROM rooms;

-- Supprimer les clients existants pour repartir à zéro
DELETE FROM clients;
...
-- Mettre à jour les usagers avec les données complètes du seed.sql (utiliser les vrais IDs)
UPDATE usagers SET 
...
-- Créer tous les clients d'abord
INSERT INTO clients (nom, prenom, email, telephone, adresse, ville, code_postal, date_naissance, type_client, notes) VALUES
('Dubois', 'Jean', 'jean.dubois@email.com', '06.12.34.56.78', '123 rue de la République, 75011 Paris', 'Paris', '75011', '1985-03-15', 'particulier', 'Client fidèle - prix conventionné'),
('Martin', 'Marie', 'marie.martin@email.com', '06.98.76.54.32', '45 avenue Jean Jaurès, 75019 Paris', 'Paris', '75019', '1992-07-22', 'particulier', 'Famille avec enfants'),
...
-- Créer des réservations de test avec les vrais IDs d'usagers
INSERT INTO reservations (usager_id, hotel_id, operateur_id, usager, date_arrivee, date_depart, nombre_personnes, statut, prix_total, prescripteur, duree, notes, client_id, nombre_adultes, nombre_enfants, chambre_id) VALUES
(41, 1, 13, 'Jean Dubois', '2024-01-15', '2024-02-15', 1, 'EN_COURS', 1395.00, 'SIAO 75', 31, 'Accompagnement social en cours', 1, 1, 0, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '101' LIMIT 1)),
(42, 1, 13, 'Marie Martin', '2024-01-20', '2024-03-20', 3, 'EN_COURS', 2700.00, 'SIAO 75', 60, 'Famille avec 2 enfants', 2, 1, 2, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '102' LIMIT 1)),
(43, 1, 14, 'Pierre Bernard', '2024-01-10', '2024-01-25', 2, 'TERMINEE', 675.00, 'Emmaüs', 15, 'Séjour terminé avec succès', 3, 1, 1, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '201' LIMIT 1)),
(44, 1, 15, 'Sophie Thomas', '2024-02-01', '2024-02-28', 1, 'EN_COURS', 1820.00, 'Secours Catholique', 28, 'Chambre double', 4, 1, 0, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '301' LIMIT 1));
