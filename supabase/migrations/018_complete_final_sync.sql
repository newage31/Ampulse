-- Migration finale complète pour synchroniser les données avec seed.sql
-- Gestion correcte de tous les IDs et relations
...
-- Créer des réservations de test avec des références aux chambres nouvellement créées
...
(1, 1, 13, 'Jean Dubois', '2024-01-15', '2024-02-15', 1, 'EN_COURS', 1395.00, 'SIAO 75', 31, 'Accompagnement social en cours', 1, 1, 0, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '101' LIMIT 1)),
(2, 1, 13, 'Marie Martin', '2024-01-20', '2024-03-20', 3, 'EN_COURS', 2700.00, 'SIAO 75', 60, 'Famille avec 2 enfants', 2, 1, 2, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '102' LIMIT 1)),
(3, 1, 14, 'Pierre Bernard', '2024-01-10', '2024-01-25', 2, 'TERMINEE', 675.00, 'Emmaüs', 15, 'Séjour terminé avec succès', 3, 1, 1, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '201' LIMIT 1)),
(4, 1, 15, 'Sophie Thomas', '2024-02-01', '2024-02-28', 1, 'EN_COURS', 1820.00, 'Secours Catholique', 28, 'Chambre double', 4, 1, 0, (SELECT id FROM rooms WHERE hotel_id = 1 AND numero = '301' LIMIT 1));
